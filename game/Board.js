...
class MinesweeperBoard {
  ...
  async render() {
    ...
    try {
      var baseimg = new Jimp(16 * ($t.width + 2), 16 * ($t.height + 2));
      var borderRightEdge = ($t.width + 1) * 16;
      var borderBottomEdge = ($t.height + 1) * 16;

      // Border corners
      var cornerIcon = textures.getBorderCorner();
      ...
      for (var y = 0; y < $t.height; y++) {
        var numberIcon = textures.getBorder(y + 1);
      ...
      for (var x = 0; x < $t.width; x++)
        for (var y = 0; y < $t.height; y++) {
          var cell = $t.get(x, y);
          var view = $t.calculateCurrentCellView(textures, cell, $t.exploded);
  ...
  calculateCurrentCellView(textures, cell, showExploded = true) {
    ...
    if (cell.number == Number.MAX_SAFE_INTEGER) return textures.loweredCell();
    return textures.getNumberWithPaletteColor(cell.number);
  }
}
/**
 * Thx stackoverflow
