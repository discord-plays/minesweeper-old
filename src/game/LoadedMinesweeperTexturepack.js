...
class LoadedMinesweeperTexturepack extends LoadedTexturepack {
  ...
  async getNumber(n, a = [null, null]) {
    ...
    var img = null;
    if ([a[0], a[1]].filter((x) => x === null).length === 0) {
      // is it a fraction?
  ...
  async getNegative(n, a = [null, null]) {
    // number, [numerator, denomenator]
    if (n === null || n === undefined) return null;
    var img = null;
    if ([a[0], a[1]].filter((x) => x === null).length === 0) {
...
module.exports = LoadedMinesweeperTexturepack;
